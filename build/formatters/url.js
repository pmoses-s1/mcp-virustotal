...
export function formatUrlScanResults(data) {
    try {
        ...
        if (attributes?.last_http_response_code) {
            outputArray.push(``, `🌐 HTTP Response:`, `• Status Code: ${attributes.last_http_response_code}`, `• Content Length: ${formatSize(attributes.last_http_response_content_length || 0)}`, attributes.last_http_response_content_sha256 ?
        ...
        if (attributes?.categories && Object.keys(attributes.categories).length > 0) {
            outputArray.push(``, `🏷️ Categories:`, ...Object.entries(attributes.categories).map(([service, category]) => `• ${service}: ${category}`));
        }
        // Add redirection chain if available
        if (redirectionChain.length > 0) {
            outputArray.push(``, `↪️ Redirection Chain:`, ...redirectionChain.map((url, index) => `${index + 1}. ${url}`));
        }
        // Add outgoing links if available
        if (outgoingLinks.length > 0) {
            outputArray.push(``, `🔗 Outgoing Links:`, ...outgoingLinks.slice(0, 5).map((url) => `• ${url}`), outgoingLinks.length > 5 ?
        ...
        if (attributes?.targeted_brand && Object.keys(attributes.targeted_brand).length > 0) {
            outputArray.push(``, `🎯 Targeted Brands:`, ...Object.entries(attributes.targeted_brand).map(([source, brand]) => `• ${source}: ${brand}`));
        }
        ...
        if (attributes?.html_meta && Object.keys(attributes.html_meta).length > 0) {
            ...
            if (metaEntries.length > 0) {
                outputArray.push(``, `📝 Meta Information:`);
        ...
        if (attributes?.favicon) {
            outputArray.push(``, `🖼️ Favicon:`, `• Hash: ${attributes.favicon.dhash}`, `• MD5: ${attributes.favicon.raw_md5}`);
        }
        ...
        if (attributes?.last_http_response_headers &&
            ...
            if (relevantHeaders.length > 0) {
                outputArray.push(``, `📋 Important HTTP Headers:`);
        ...
        if (attributes?.last_http_response_cookies &&
            ...
            outputArray.push(``, `🍪 Cookies:`, ...Object.entries(attributes.last_http_response_cookies)
