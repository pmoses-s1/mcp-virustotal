...
export function formatBehaviourSummary(hash, data) {
    try {
        if (!data) {
        ...
        lines.push(...section('🔧 Processes Created', data.processes_created, (p) => String(p)), ...section('💉 Processes Injected', data.processes_injected, (p) => String(p)), ...section('💀 Processes Terminated', data.processes_terminated, (p) => String(p)), ...section('⌨️  Command Executions', data.command_executions, (c) => String(c)), ...section('📂 Files Opened', data.files_opened, (f) => String(f)), ...section('✏️  Files Written', data.files_written, (f) => String(f)), ...section('📦 Files Dropped', data.files_dropped, (f) => `${f.path || ''} ${f.sha256 ? `(${f.sha256})` : ''}`.trim()), ...section('🗑️  Files Deleted', data.files_deleted, (f) => String(f)), ...section('🔑 Registry Keys Opened', data.registry_keys_opened, (r) => String(r)), ...section('📝 Registry Keys Set', data.registry_keys_set, (r) => `${r.key || ''} = ${r.value || ''}`.trim()), ...section('🧹 Registry Keys Deleted', data.registry_keys_deleted, (r) => String(r)), ...section('🌐 DNS Lookups', data.dns_lookups, (d) => `${d.hostname || ''}${d.resolved_ips?.length ? ` → ${d.resolved_ips.join(', ')}` : ''}`.trim()), ...section('📡 IP Traffic', data.ip_traffic, (t) => `${t.transport_layer_protocol || ''} ${t.destination_ip || ''}:${t.destination_port || ''}`.trim()), ...section('🌍 HTTP Conversations', data.http_conversations, (h) => `${h.request_method || 'GET'} ${h.url || ''}`.trim()), ...section('🔒 Mutexes Created', data.mutexes_created, (m) => String(m)), ...section('📚 Modules Loaded', data.modules_loaded, (m) => String(m)));
        if (data.ids_results?.length) {
            ...
            for (const r of data.ids_results.slice(0, PREVIEW)) {
                ...
                lines.push(`  - ${r.rule_msg || r.rule_id || 'alert'} [${r.alert_severity || 'info'}]`, `      ${ctx.proto || ''} ${ctx.src_ip || ''}:${ctx.src_port || ''} → ${ctx.dest_ip || ''}:${ctx.dest_port || ''}`.replace(/\s+/g, ' ').trim());
            }
