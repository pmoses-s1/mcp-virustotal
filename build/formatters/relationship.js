 ...
 */
export function formatRelationshipPage(opts) {
    try {
        ...
        const counts = total != null && total !== items.length
...
export function formatRelationshipResults(data, type) {
    try {
                ...
                try {
                    switch (type) {
                        ...
                        case 'domain':
                            ...
                            if (item?.attributes?.hostname)
                                return `• ${item.attributes.hostname}`;
                            if (item?.attributes?.value)
                                return `• ${item.attributes.value}`;
                            if (item?.attributes?.domain)
                                return `• ${item.attributes.domain}`;
                            if (item?.attributes?.ip_address)
                                return `• ${item.attributes.ip_address}`;
                            if (item?.attributes?.date)
                                return `• Record from ${item.attributes.date}`;
