    ...
    switch (relType) {
        case 'behaviours':
            ...
            const registry = [
                ...
                ...(attrs.registry_keys_set?.map((reg) => `${reg.key}: ${reg.value}`) || []),
                ...(attrs.registry_keys_deleted || [])
            ].filter(Boolean);
            const network = attrs.ids_results?.map((result) => {
...
export function formatFileResults(data) {
    try {
        ...
        outputArray.push(``, `👥 Community Feedback:`, `• Reputation Score: ${reputation}`, `• Harmless Votes: ${votes.harmless || 0}`, `• Malicious Votes: ${votes.malicious || 0}`);
        // Add capabilities if available
        if (attributes?.capabilities_tags?.length > 0) {
            outputArray.push(``, `⚡ Capabilities:`, ...attributes.capabilities_tags.map((tag) => `• ${formatCapabilityTag(tag)}`));
        }
        // Add tags if available
        if (attributes?.tags?.length > 0) {
            outputArray.push(``, `🏷️ Tags:`, ...attributes.tags.map((tag) => `• ${tag}`));
        }
        // Add sandbox verdicts if available
        if (Object.keys(sandboxResults).length > 0) {
            outputArray.push(``, `🔬 Sandbox Analysis Results:`);
            for (const [sandbox, verdict] of Object.entries(sandboxResults)) {
                const v = verdict;
                outputArray.push(`${sandbox}:`, `• Category: ${v.category}`, `• Confidence: ${v.confidence}%`, ...(v.malware_classification ? [`• Classification: ${v.malware_classification.join(', ')}`] : []), ...(v.malware_names ? [`• Identified as: ${v.malware_names.join(', ')}`] : []), ``);
            }
        }
        // Add Sigma analysis results if available
        if (sigmaResults.length > 0 || Object.keys(sigmaStats).length > 0) {
            outputArray.push(``, `🎯 Sigma Analysis:`, `Statistics:`, `• Critical: ${sigmaStats.critical || 0}`, `• High: ${sigmaStats.high || 0}`, `• Medium: ${sigmaStats.medium || 0}`, `• Low: ${sigmaStats.low || 0}`, ``);
            if (sigmaResults.length > 0) {
                outputArray.push(`Detected Rules:`);
                for (const result of sigmaResults) {
                    const r = result;
                    outputArray.push(`${r.rule_title}:`, `• Level: ${r.rule_level}`, `• Source: ${r.rule_source}`, `• Description: ${r.rule_description}`, `• Author: ${r.rule_author}`, ``);
                }
            }
        }
        // Add crowdsourced IDS results if available
        if (crowdsourcedIds.length > 0) {
            outputArray.push(``, `🛡️ Intrusion Detection Results:`, `Statistics:`, `• High: ${crowdsourcedIdsStats.high || 0}`, `• Medium: ${crowdsourcedIdsStats.medium || 0}`, `• Low: ${crowdsourcedIdsStats.low || 0}`, `• Info: ${crowdsourcedIdsStats.info || 0}`, ``);
            outputArray.push(`Alerts:`);
            for (const alert of crowdsourcedIds) {
                const a = alert;
                outputArray.push(`• ${a.rule_msg}`, `  - Severity: ${a.alert_severity}`, `  - Category: ${a.rule_category}`, `  - Source: ${a.rule_source}`, ``);
            }
        }
        // Add YARA results if available
        if (yaraResults.length > 0) {
            outputArray.push(``, `🔍 YARA Detections:`);
            for (const yara of yaraResults) {
                const y = yara;
                outputArray.push(`• Rule: ${y.rule_name}`, `  - Description: ${y.description}`, `  - Ruleset: ${y.ruleset_name}`, `  - Source: ${y.source}`, ``);
            }
        }
        ...
        if (popularEngines.length > 0) {
            outputArray.push(``, `🔰 Popular Engines Results:`, ...popularEngines.map(([engine, result]) => `• ${engine}: ${result?.result || 'Clean'} ${result?.category === 'malicious' ? '🔴' :
...
function formatCapabilityTag(tag) {
